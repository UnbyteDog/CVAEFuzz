...
class SQLiEngine(BaseEngine):
    ...
    REFLECTION_PATTERNS = [
        # HTML标签内的反射（value、placeholder等属性）
        r'value\s*=\s*["\'][^"\']*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',
        r'placeholder\s*=\s*["\'][^"\']*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',

        # HTML内容直接反射（不在标签内）
        r'<(?:div|span|p|td|li|h[1-6])[^>]*>(?:\s|&nbsp;)*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',

        # JavaScript字符串内的反射
        r'(?:var|let|const)\s+\w+\s*=\s*["\'][^"\']*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',
        r'["\'][^"\']*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)[^"\']*["\']',

        # 表单字段的反射（input、textarea）
        r'<(?:input|textarea|select)[^>]+name\s*=\s*["\'][^"\']*["\'][^>]*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',

        # URL参数中的反射（罕见但可能）
        r'[?&]\w+\s*=\s*(?:\%27|\'|\"|;|\-\-|and\s+\d+\=\d|or\s+\d+\=\d|union\s+select)',

        # 通用SQL注入关键词反射（直接匹配payload特征）
        r'(?:and|or)\s+\d+\s*(?:=|!=|<>|<|>)\s*\d',
        r'union\s+select',
    ...
    def _detect_logic_based_blind(self,
        ...
        for true_pattern, false_pattern in logic_pairs:
            try:
                ...
                if true_result2:
                    true_response2 = true_result2['response']
                    true_length2 = len(true_response2.text)

                    logger.info(f"[SQLi] [逻辑-盲注] 第二次True响应: length={true_length2}")

                    # 计算增长量
                    growth_1_to_2 = false_length - true_length   # 第一次True → False
                    growth_2_to_3 = true_length2 - false_length  # False → 第二次True
                ...
                if true_hash != false_hash:
                    ...
                    if 0.95 < similarity < 0.999:
                        ...
                        logger.warning(
                            f"[SQLi] [逻辑-盲注] 发现漏洞: {param_name}={true_injected[:40]}... "
                            f"(相似度={similarity:.4f}, 置信度={confidence:.2f})"
                        )
        ...
        for true_pattern, false_pattern in logic_pairs:
            try:
                ...
                if true_result2:
                    ...
                    logger.info(f"[SQLi] [逻辑] 第二次True响应: length={true_length2}")

                    # 计算增长量
                    growth_1_to_2 = false_length - true_length
                    growth_2_to_3 = true_length2 - false_length
                    ...
                    if growth_1_to_2 > 0 and growth_2_to_3 > 0:
                        growth_ratio = min(growth_1_to_2, growth_2_to_3) / max(growth_1_to_2, growth_2_to_3)
                        if growth_ratio > 0.8:
                            logger.warning(f"[SQLi] [逻辑] ❌ 检测到累积增长模式（疑似留言板），跳过该对！")
                            logger.info(f"[SQLi] [逻辑] 增长相似度={growth_ratio:.2%}，判定为累积显示而非SQL注入")
                            continue
    ...
    def _is_payload_reflected(self, response_text: str, payload: str) -> bool:
        ...
        检测策略：
        1. 检查响应中是否包含payload的SQL注入特征
        2. 匹配反射特征库（HTML标签、JS字符串、表单字段等）
        3. 如果匹配到反射特征，说明payload只是被反射，不是真正的SQL注入

        Args:
            response_text: HTTP响应内容
            payload: 原始payload

        Returns:
            True表示payload被反射（可能是误报），False表示可能是真注入
        """
        ...
        for feature in sql_injection_features:
            if feature in payload_core and feature in response_lower:
                ...
                sql_error_indicators = [
                    ...
                    "postgresql error", "mssql error", "sqlite error",
                    "odbc error", "jdbc error", "syntax error"
                ]
            ...
            for pattern in self.REFLECTION_PATTERNS:
                try:
                    if re.search(pattern, response_text, re.IGNORECASE):
                        logger.info(f"[SQLi] [反射检测] 匹配到反射特征: {pattern[:50]}")
                        return True
                except re.error as e:
                    logger.warning(f"[SQLi] [反射检测] 正则表达式错误: {pattern} - {e}")
                    continue
        except Exception as e:
        ...
        if len(payload_words) >= 3:  # 至少3个单词才计算相似度
            ...
            if payload_words.issubset(response_words):
                # payload的所有单词都在响应中
                logger.info(f"[SQLi] [反射检测] payload词汇全部出现在响应中")
                return True
